...
namespace hilbert {
    ...
    EOM_EE_CCSD::EOM_EE_CCSD(std::shared_ptr<CC_Cavity> &cc_wfn, Options &options) :
...
//        ground_energy_ref_ = eigvals_->get(0);
    ...
    void EOM_EE_CCSD::pack_sigma_vectors(size_t L, double **sigmar, double **sigmal) {
        ...
        // ground state
        {
            foreach_inplace(sigvec_blks_["sigmar0"], [sigmar](auto &tile) {
        ...
        // singles
        {
            foreach_inplace(sigvec_blks_["sigmar1_aa"], [sigmar, oa, offset](auto &tile) {
                for (auto &x : tile.range())
                    sigmar[x[1]*oa + x[2] + offset][x[0]] = tile[x];
            }); offset += aa;
            
            foreach_inplace(sigvec_blks_["sigmar1_bb"], [sigmar, ob, offset](auto &tile) {
                for (auto &x : tile.range())
        ...
        // doubles
        {
            ...
            foreach_inplace(sigvec_blks_["sigmar2_aaaa"], [sigmar, oa, va, offset](auto &tile) {
                for (auto &x : tile.range()) {
                    if (x[1] < x[2] && x[3] < x[4]) {
            ...
            foreach_inplace(sigvec_blks_["sigmar2_abab"], [sigmar, oa, ob, vb, offset](auto &tile) {
                for (auto &x : tile.range())
                    sigmar[x[1]*vb*oa*ob + x[2]*oa*ob + x[3]*ob + x[4] + offset][x[0]] = tile[x];
            }); offset += abab;
            
            foreach_inplace(sigvec_blks_["sigmar2_bbbb"], [sigmar, ob, vb, offset](auto &tile) {
                for (auto &x : tile.range()) {
                    if (x[1] < x[2] && x[3] < x[4]) {
        ...
        // ground state
        {
            foreach_inplace(sigvec_blks_["sigmal0"], [sigmal](auto &tile) {
                for (auto &x : tile.range())
                    sigmal[0][x[0]] = tile[x];
            }); offset += 1;
        }
        // singles
        {
            foreach_inplace(sigvec_blks_["sigmal1_aa"], [sigmal, oa, offset](auto &tile) {
                for (auto &x : tile.range())
                    sigmal[x[1]*oa + x[2] + offset][x[0]] = tile[x];
            }); offset += aa;

            foreach_inplace(sigvec_blks_["sigmal1_bb"], [sigmal, ob, offset](auto &tile) {
                for (auto &x : tile.range())
        ...
        // doubles
        {
            foreach_inplace(sigvec_blks_["sigmal2_aaaa"], [sigmal, oa, va, offset](auto &tile) {
                for (auto &x : tile.range()) {
                    if (x[1] < x[2] && x[3] < x[4]) {
                        size_t ab_off = EOM_Driver::sqr_2_tri_idx(x[1], x[2], va);
                        size_t ij_off = EOM_Driver::sqr_2_tri_idx(x[3], x[4], oa);
            ...
            foreach_inplace(sigvec_blks_["sigmal2_abab"], [sigmal, oa, ob, vb, offset](auto &tile) {
                for (auto &x : tile.range())
                    sigmal[x[1] * vb * oa * ob + x[2] * oa * ob + x[3] * ob + x[4] + offset][x[0]] = tile[x];
            }); offset += abab;

            foreach_inplace(sigvec_blks_["sigmal2_bbbb"], [sigmal, ob, vb, offset](auto &tile) {
                for (auto &x : tile.range()) {
                    if (x[1] < x[2] && x[3] < x[4]) {
                        size_t ab_off = EOM_Driver::sqr_2_tri_idx(x[1], x[2], vb);
                        size_t ij_off = EOM_Driver::sqr_2_tri_idx(x[3], x[4], ob);
    ...
    EOM_Driver::DominantTransitionsType EOM_EE_CCSD::find_dominant_transitions(size_t I) {
        ...
        for (size_t a = 0; a < va_; a++) {
            for (size_t i = 0; i < oa_; i++) {
                ...
                lr = l*r; // get lr
                if (fabs(lr) > threshold) {
                    dominant_transitions["l1*r1"].push({lr, l, r, "aa", {a+1 + oa_, oa_ - i}});
                }
                id++;
            }
        }
        ...
        for (size_t a = 0; a < vb_; a++) {
            for (size_t i = 0; i < ob_; i++) {
                ...
                lr = l*r; // get lr
                if (fabs(lr) > threshold) {
                    dominant_transitions["l1*r1"].push({lr, l, r, "bb", {a+1 + ob_, ob_ - i}});
                }
                id++;
            }
        }
            ...
            for (size_t b = a + 1; b < va_; b++) {
                for (size_t i = 0; i < oa_; i++) {
                    for (size_t j = i + 1; j < oa_; j++) {
                        ...
                        lr = l*r; // get lr
                        if (fabs(lr) > threshold) {
                            dominant_transitions["l2*r2"].push({lr, l, r, "aaaa", {a+1 + oa_, b+1 + oa_, oa_ - i, oa_ - j}});
                        }
                        id++;
                    }
                }
            ...
            for (size_t b = 0; b < vb_; b++) {
                for (size_t i = 0; i < oa_; i++) {
                    for (size_t j = 0; j < ob_; j++) {
                        ...
                        lr = l*r; // get lr
                        if (fabs(lr) > threshold) {
                            dominant_transitions["l2*r2"].push({lr, l, r, "abab", {a+1 + oa_, b+1 + ob_, oa_ - i, ob_ - j}});
                        }
                        id++;
                    }
                }
            ...
            for (size_t b = a + 1; b < vb_; b++) {
                for (size_t i = 0; i < ob_; i++) {
                    for (size_t j = i + 1; j < ob_; j++) {
                        ...
                        lr = l*r; // get lr
                        if (fabs(lr) > threshold) {
                            dominant_transitions["l2*r2"].push({lr, l, r, "bbbb", {a+1 + ob_, b+1 + ob_, ob_ - i, ob_ - j}});
                        }
                        id++;
                    }
                }
