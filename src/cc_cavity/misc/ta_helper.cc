...
template<typename T>
inline TArray<T> TA_Helper<T>::makeTensor(World &world, const initializer_list<size_t> &N, bool fillZero){
...
template<typename T>
TiledRange TA_Helper<T>::makeRange(const initializer_list<size_t> &N) {// create range
    vector<size_t> Nblk;
...
template<typename T>
TiledRange TA_Helper<T>::makeRange(const vector<size_t> &N) {// create range
    vector<size_t> Nblk;
    vector<TiledRange1> trange;
...
template<typename T>
inline TArray<T> TA_Helper<T>::makeTensor(World &world, const initializer_list<size_t> &N, const T *data,
    ...
    if (Off.size() == 0){ // if Off is empty
        ...
        // fill tensor with data
        array.template init_elements([data, N_size, off_it, dim_sizes](const typename TArray<T>::index &I) { // fill tensor with data
                    size_t index = 0;
                    // calculate index
                    for (size_t i = 0; i < N_size; i++){
...
template<typename T>
inline TArray<T> TA_Helper<T>::makeTensor(World &world,
    ...
    if (Off.size() == 0){ // if Off is empty
        ...
        // fill tensor with data
        array.template init_elements([data, NL_size, NR_size, dim_sizes_L, dim_sizes_R](const typename TArray<T>::index &I) {
        ...
        // fill tensor with data
        array.template init_elements([data, NL_size, NR_size, dim_sizes_L, dim_sizes_R, off_it](const typename TArray<T>::index &I) { // fill tensor with data
                                        size_t indexL = 0, indexR = 0;
                                        for (size_t i = 0; i < NL_size; i++)
...
template<typename T>
inline void TA_Helper<T>::forall(TArray<T> &tensor, function<void(Tensor<T> &, vector<size_t> &)> op) {
    // get rank of tensor
...
template<typename T>
T * TA_Helper<T>::arrayFromTensor(TArray<T> tensor, const initializer_list<size_t> &N, initializer_list<size_t> Off) {
