...
void PolaritonicRTDDFT::build_sigma_generalized(int N, int maxdim, int L, double **Q, double **sigmah, double **sigmas){
    ...
    double *sigma_m = (double*)malloc(L*sizeof(double));
    ...
    memset((void*)sigma_m,'\0',L*sizeof(double));
    ...
    build_gm(N, L, m, gm);
    build_gm(N, L, n, gn);
    build_sigma_m(N, L, x, y, m, sigma_m);
...
/**
 *
 * build photon part of electronic sigma vector
 *
 */
 ...
 *
 */
void PolaritonicRTDDFT::build_sigma_m(int N, int L, double *x, double *y, double *m, double *sigma_m){
