...
OrbitalOptimizer::OrbitalOptimizer(std::shared_ptr<Wavefunction> reference_wavefunction, Options & options){
    ...
    }else if ( reference_wavefunction->options().get_str("SCF_TYPE") == "DF" ) {
    ...
    oo_active_active_rotations_ = active_active_rotations_;;
    oo_maxiter_                 = maxiter_;
    oo_energy_convergence_      = e_convergence_;
    oo_gradient_convergence_    = g_convergence_;
