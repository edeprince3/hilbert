...
void OrbitalOptimizer::optimize_orbitals(double * d2, double * d1, double * tei, double * oei, double * transformation_matrix,\
    ...
    for ( int iter = 0; iter < oo_maxiter_; iter++){
        ...
        if ( gradient_norm_ < oo_gradient_convergence_ ) converged_g = true;
        if ( fabs(dE) < oo_energy_convergence_ )         converged_e = true;

        iter_orbopt++;

        if ( converged_e || converged_g || iter == oo_maxiter_ - 1 ) break;
