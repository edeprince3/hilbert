...
#include <polaritonic_scf/rdfrpa.h>
...
int read_options(std::string name, Options& options)
{
    if (name == "HILBERT"|| options.read_globals()) {
        ...
        options.add_str("HILBERT_METHOD", "", "DOCI P2RDM PP2RDM V2RDM_DOCI V2RDM_CASSCF JELLIUM_SCF POLARITONIC_RHF POLARITONIC_UHF POLARITONIC_ROHF POLARITONIC_UKS POLARITONIC_RKS POLARITONIC_RCIS POLARITONIC_UCCSD POLARITONIC_RTDDFT POLARITONIC_UTDDFT POLARITONIC_RPA POLARITONIC_RDFRPA MCPDFT");
...
SharedWavefunction hilbert(SharedWavefunction ref_wfn, Options& options)
{
    ...
    }else if ( options.get_str("HILBERT_METHOD") == "POLARITONIC_RDFRPA") {

        std::shared_ptr<PolaritonicRKS> rks (new PolaritonicRKS(ref_wfn,options));
        double energy = rks->compute_energy();

        std::shared_ptr<PolaritonicRDFRPA> dfrpa (new PolaritonicRDFRPA((std::shared_ptr<Wavefunction>)rks,options,ref_wfn));
        double dum = dfrpa->compute_energy();
