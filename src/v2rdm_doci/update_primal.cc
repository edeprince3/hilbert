    ...
    for (int h = 0; h < nirrep_; h++) {
        ...
        for (int i = 0; i < amopi_[h]; i++) {
            for (int j = 0; j < amopi_[h]; j++) {
                ...
                for (int k = 0; k < amopi_[h]; k++) {
                    duma += x_p[d1off[h] + i*amopi_[h] + k] * t_p[j+frzcpi_[h]+rstcpi_[h]][k+frzcpi_[h]+rstcpi_[h]];
                    dumb += x_p[d1boff[h] + i*amopi_[h] + k] * t_p[j+frzcpi_[h]+rstcpi_[h]][k+frzcpi_[h]+rstcpi_[h]];
                }
                tmp_p[d1off[h] + i*amopi_[h] + j] = duma;
        ...
        for (int i = 0; i < amopi_[h]; i++) {
            for (int j = 0; j < amopi_[h]; j++) {
                ...
                double dumb = 0.0;
                for (int k = 0; k < amopi_[h]; k++) {
                    duma += tmp_p[d1off[h] + k*amopi_[h] + i] * t_p[j+frzcpi_[h]+rstcpi_[h]][k+frzcpi_[h]+rstcpi_[h]];
                    dumb += tmp_p[d1boff[h] + k*amopi_[h] + i] * t_p[j+frzcpi_[h]+rstcpi_[h]][k+frzcpi_[h]+rstcpi_[h]];
                }
                x_p[d1off[h] + j*amopi_[h] + i] = duma;
