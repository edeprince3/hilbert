...
void v2RDMSolver::Generalized_Pauli_3_8_constraints_ATu(SharedVector A,SharedVector u, int state){
    ...
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,3, -1, -1, -1, -1, -1, -1, 3); //= 1,  ## Extended Pauli inequality: lambda[1]+lambda[8]<=1 ##

    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,-1, 1, 1, 1, 1, -1, -1, -1); //= 1, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,1, 1, -1, -1, 1, 1, -1, -1); //= 1, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,1, 1, -1, 1, -1, -1, 1, -1); //= 1, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,1, -1, 1, 1, -1, 1, -1, -1); //= 1, 

    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,2, 1, -2, -1, 0, -1, 0, 1); //= 1, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,2, -1, 0, -1, 0, 1, -2, 1); //= 1, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,0, 0, 1, 2, -2, -1, -1, 1); //= 1, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,1, 2, -2, 0, -1, -1, 0, 1); //= 1, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,2, -1, 0, 1, -2, -1, 0, 1); //= 1, 

    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,5, 5, -7, -3, -3, 1, 1, 1); //= 3, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,5, -3, -3, 1, 1, 5, -7, 1); //= 3, 

    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,5, 1, -3, 1, -3, 1, -3, 1); //= 3, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,1, 1, 1, 5, -3, -3, -3, 1); //= 3, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,1, 5, -3, 1, 1, -3, -3, 1); //= 3, 


    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,9, 1, -7, -7, -7, 1, 1, 9); //= 3, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,9, -7, -7, 1, 1, 1, -7, 9); //= 3, 

    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,7, -1, -1, -1, -1, 7, -9, -1); //= 5, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,7, -1, -1, 7, -9, -1, -1, -1); //= 5, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,7, 7, -9, -1, -1, -1, -1, -1); //= 5,
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,-1, -1, 7, 7, -1, -1, -9, -1); //= 5,  
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,-1, 7, -1, 7, -1, -9, -1, -1); //= 5, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,-1, 7, -1, -1, 7, -1, -9, -1); //= 5, 

    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,-3, 5, 5, 13, -11, -3, -11, 5); //= 7, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,5, 13, -11, 5, -11, -3, -3, 5); //= 7, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,5, -3, 5, 13, -11, -11, -3, 5); //= 7, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,5, 13, -11, -3, 5, -11, -3, 5); //= 7, 

    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,19, 11, -21, -13, -5, -5, 3, 11); //= 9, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,19, -13, -5, -5, 3, 11, -21, 11); //= 9, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,11, 19, -21, -5, -13, -5, 3, 11); //= 9, 
    GP_N_8_ATu(u_p[offset++],off,A_p,gpc_rdm_map_a_[state],gpc_rdm_map_b_[state],orb_p,-5, 3, 11, 19, -21, -13, -5, 11); //= 9]:
}
...
void v2RDMSolver::GP_N_8_ATu(double dum,int & off, double * A, int *** map_a, int *** map_b, double ** orbs, 
