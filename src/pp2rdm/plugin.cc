 ...
 * @BEGIN LICENSE
 *
 * pp2RDM by Psi4 Developer, a plugin to:
 *
...
{
    if (name == "PP2RDM"|| options.read_globals()) {
...
extern "C" PSI_API
SharedWavefunction pp2rdm(SharedWavefunction ref_wfn, Options& options)
{
    std::shared_ptr<pp2RDMSolver> pp2rdm (new pp2RDMSolver(ref_wfn,options));
    double energy = pp2rdm->compute_energy();
    double excited_energy=pp2rdm->compute_excited_energy(); 
