...
namespace psi {
    void
    GeneralizedEigenvalueProblem(long int N, double **A, double **B, double **vr, double *eig, bool *energy_is_real) {
        ...
        INFO = C_DGGEV(JOBVL, JOBVR, N, Asp[0], LDA, Bsp[0], LDB, ALPHAR, ALPHAI, BETA, VL[0], LDVL, VR[0], LDVR, WORK,
                       LWORK);
    ...
    void
    NonSymmetricEigenvalueEigenvector(long int dim, double **M, double *eigval, double *wi, double **el, double **er) {
        long int info;
        char vl = 'V';
        char vr = 'V';
        long int n = dim;
    ...
    void Nonsym_DavidsonSolver::solve(double *Adiag, int N, int M, double *reval, double **rer, double **rel,
                                      BuildSigma build_sigma, int maxdim, size_t init_dim, double residual_norm,
                                      bool use_residual_norm) {
        int min_pos;
        double minimum;
        //int N = o*v;
        ...
        while (iter < maxit) {
            ...
            outfile->Printf("%9s %20s %20s\n", "    ", "error", "eigenvalue");
            for (int k = 0; k < Mn; k++) {
                double error = lambdap[k] - lambda_oldp[k];
                outfile->Printf("root%5d %20.12lf %20.12lf \n", k, error, lambdap[k]);
            }
            ...
            for (int k = 0; k < Mn; k++) {
                ...
                bool e_conv = fabs(e_error) < eom_e_conv;
                ...
                r_resid_conv |= normvalr < eom_r_conv;
                l_resid_conv |= normvall < eom_r_conv;
            ...
            outfile->Printf("converged %d roots\n", conv);
    ...
    int Nonsym_DavidsonSolver::writeVectors(int N, int M, int maxdim, double *reval, double **rer, double **rel,
        ...
        if (M > maxdim) {
            outfile->Printf(
                    "WARNING: %d complex eigenvalues encountered. Subspace is insufficient to store them and provide the requested roots.\n",
                    num_i);
        }
        M = M_i;
        return M;
    ...
    void Nonsym_DavidsonSolver::real_generalized_eigenvalue_problem(double *Hdiag, double *Sdiag, size_t N, size_t M,
                                                                    double *reval, double **rer,
                                                                    BuildSigma2 build_sigma2, size_t maxdim,
                                                                    size_t init_dim, double residual_norm,
                                                                    bool use_residual_norm) {
        double minimum;
        int min_pos;
    ...
    void Nonsym_DavidsonSolver::schmidt_biorthogonal(double **A, double **B, size_t m, size_t n) {
        ...
        for (size_t i = 0; i < m; i++) {
            ...
            if (fabs(RValue1) < 1e-6) {
                outfile->Printf("vector A[%d] and  B[%d] are already orthogonal.", i,
                                i);//possibly need to choose new B[i] from other rows so that A[i] *B[i] \neq 0
                exit(0);
            }
            for (size_t I = 0; I < n; I++) {
                if (RValue1 < 0.0) {
